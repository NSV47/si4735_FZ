...
uint8_t si4734_fm_mode(){
    ...
    furi_hal_i2c_tx(&furi_hal_i2c_handle_external, SI4734ADR, cmd, 3, timeout); // i2c_transfer7(SI4734I2C,SI4734ADR,cmd,3,0,0);
	delay(1000); // furi_delay_ms(1000); // delay(1000);
#if 1	
    do{ 
		furi_hal_i2c_rx(&furi_hal_i2c_handle_external, SI4734ADR, &status, 1, timeout); // i2c_transfer7(SI4734I2C,SI4734ADR,0,0,&status,1);
		tray++;
		if(tray==255){
